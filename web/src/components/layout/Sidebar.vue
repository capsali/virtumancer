...
const expandedDatacenter = ref(true); // Start expanded by default
...
function toggleDatacenter() {
    expandedDatacenter.value = !expandedDatacenter.value;
}
...
const hostStateText = (host) => {
    ...
    // Prefer authoritative server-side `state` when present, then any `info.connected` flag returned by host info endpoint, then the `connected` boolean.
    const states = {
        'CONNECTED': 'Connected',
    ...
    if (host.info && typeof host.info.connected !== 'undefined') return host.info.connected ? 'Connected' : 'Disconnected';
...
const hostStateColor = (host) => {
  ...
  if (host.info && typeof host.info.connected !== 'undefined') return host.info.connected ? 'text-green-400 bg-green-900/50' : 'text-gray-400 bg-gray-700';
    ...
    <nav class="flex-1 overflow-y-auto">
      <ul class="px-4">
        <!-- Datacenter Root -->
                <!-- Datacenter Root -->
        <li class="mb-2">
            <div @click="selectDatacenter" class="flex items-center p-2 rounded-md cursor-pointer hover:bg-gray-700" :class="{ 'bg-gray-700 text-white': !mainStore.selectedHostId && $route.name === 'datacenter' }">
                <button @click.stop="toggleDatacenter" v-show="uiStore.isSidebarOpen" class="mr-2 focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform" :class="{'rotate-90': expandedDatacenter}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                </button>
            ...
            <!-- Hosts under datacenter -->
            <div v-if="expandedDatacenter && uiStore.isSidebarOpen" class="mt-1 ml-6 border-l-2 border-gray-700 pl-4">
                <div class="text-xs text-gray-400 uppercase font-semibold mb-2">Hosts</div>
                <ul class="space-y-1">
                    <li v-for="host in mainStore.hosts" :key="host.id" class="mb-1">
                        ...
                        >
                            <button @click.stop="toggleHost(host.id)" class="mr-2 focus:outline-none">
                            ...
                            <svg class="h-5 w-5 flex-shrink-0" :class="{'text-indigo-400': mainStore.selectedHostId === host.id}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/></svg>
                            <span class="ml-2 font-medium truncate">{{ host.id }}</span>
                            <div class="ml-auto flex items-center space-x-2">
                                <span v-if="mainStore.visibleConnecting && mainStore.visibleConnecting[host.id]" class="text-xs px-2 py-0.5 rounded-full bg-yellow-500 text-white">connecting...</span>
                                <span v-else class="text-xs px-2 py-0.5 rounded-full" :class="hostStateColor(host)">{{ hostStateText(host) }}</span>
                                <span v-if="host.vms" class="text-xs font-mono bg-gray-800 px-2 py-0.5 rounded-full">
                        ...
                        <div v-if="expandedHosts[host.id]" class="mt-1 ml-6 border-l-2 border-gray-600 pl-4">
                            ...
                            <div v-if="mainStore.discoveredByHost[host.id] && mainStore.discoveredByHost[host.id].length" class="mb-2">
                                ...
                                <ul class="space-y-1">
                                    <li v-for="d in mainStore.discoveredByHost[host.id]" :key="d.uuid">
                                        <div class="flex items-center p-1.5 text-sm rounded-md cursor-pointer hover:bg-gray-700">
                                            <span class="h-2 w-2 rounded-full mr-2 flex-shrink-0 bg-blue-500"></span>
                                            ...
                                            <button
                                                ...
                                                :class="[
                                                    'ml-auto text-xs px-2 py-0.5 rounded text-white',
        ...
        <!-- Network Management -->
        <li class="mb-2">
            <div @click="selectNetwork" class="flex items-center p-2 rounded-md cursor-pointer hover:bg-gray-700" :class="{ 'bg-gray-700 text-white': $route.name === 'network' }">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                <span class="ml-3 font-semibold" v-show="uiStore.isSidebarOpen">Network</span>
            </div>
        </li>
